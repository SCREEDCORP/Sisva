...
export class TracingPatientComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this._patientService.onProductChanged
            ...
            .subscribe(patient => {
                ...
                this.listProductAdm = this._patientUtil.getProductAdm().filter(x => x.type === 'ges');
    ...
    createTraingForm(tracing): FormGroup {
        return this._formBuilder.group({
            ...
            trc_num_mue: [this._registryUtil.ValidarCampoNull(tracing, 'trc_num_mue')],
            trc_fec_mue: [
                { value: this._registryUtil.ValidarCampoNull(tracing, 'trc_fec_mue'), disabled: this.roleAdminReadonly }
            ],
            trc_hsd: [this._registryUtil.ValidarCampoNull(tracing, 'trc_hsd')],
            trc_hcd: [this._registryUtil.ValidarCampoNull(tracing, 'trc_hcd')],
            ...
            trc_ane_state: [this._registryUtil.ValidarCampoNull(tracing, 'trc_ane_state')],
            trc_pat_weight: [this._registryUtil.ValidarCampoNull(tracing, 'rc_pat_weight')],
            trc_pat_est_salud: [this._registryUtil.ValidarCampoNull(tracing, 'trc_pat_est_salud')],
            tr_pat_his_cli: [this._registryUtil.ValidarCampoNull(tracing, 'tr_pat_his_cli')],

            pat_ges: [this._registryUtil.ValidarCampoNull(tracing, 'pat_ges')],
            pat_paridad: [this._registryUtil.ValidarCampoNull(tracing, 'pat_paridad')],
            pat_fur: [this._registryUtil.ValidarCampoNull(tracing, 'pat_fur')],
            pat_fpp: [this._registryUtil.ValidarCampoNull(tracing, 'pat_fpp')],
            edadGestacional: [
                this.calculateEdadGestacionalRet(
                    moment(this._registryUtil.ValidarCampoNull(tracing, 'pat_fur'))
                )
            ]
        });
    }

    calculateEdadGestacional(value, form): void {
        let format = '';
        ...
        form.controls.edadGestacional.setValue(format);
    }

    calculateEdadGestacionalRet(value): string {
        let format = '';
        if (value != '') {
            const currentDate = moment();
            const fecNacDate = moment(this._registryUtil.getStringFromDate(value));
    ...
    finalizeRegistry(): void {
        this._router.navigateByUrl(
            '/tracing-ges/' + this.patient.pat_id + '/' + this.patient.pat_handle
        );
    }
                ...
                case '1':
                    if (element.schedule_ejec === '1') {
                        this.arrayTracing[0].schedule_trc_data = {
                            ...
                            pat_ges: this.tracing1Form.value.pat_ges,
                            pat_paridad: this.tracing1Form.value.pat_paridad,
                            pat_fur: this.tracing1Form.value.pat_fur,
                            pat_fpp: this.tracing1Form.value.pat_fpp,
