...
export class PatientComponent implements OnInit, OnDestroy {
    ...
    createPatientFormRegistry(): void {
        ...
        this.medicForm = this._formBuilder.group({
            ...
            pat_weight_id: ['', Validators.required],
            ...
            pat_trc_tol_trt: [false, Validators.required],
            pat_trc_vst_med: [false, Validators.required]
        });
    ...
    createPatientForm(): FormGroup {
        ...
        const pat_weight = this.listWeight.find(
            x => x.pat_weight_id === this.patient.pat_medic_info.pat_weight
        ).pat_weight_desc;
            ...
            this._ubigeoService.getListDpt().subscribe(data => {
                if (data.res_service == 'ok') {
                    if (data.data_result.Count > 0) {
                        ...
                        this.listUbigeoDpt = this.listUbigeoDpt.sort(function(
                            a,
                            b
                        ) {
    ...
    finalizeRegistry(): void {
        this._router.navigateByUrl('/patients/type/ane');
    }
        ...
        try {
            ...
            const bodyPatient = {
                ...
                pat_medic_info: {
                    ...
                    pat_adm: {
                        pat_adm_id:
                            this.medicForm.value.pat_adm_id !== ''
                                ? this.medicForm.value.pat_adm_id
                                : '0',
                        pat_adm_desc:
                            this.medicForm.value.pat_adm_id !== ''
                                ? this.listProdAdm.find(
                                      x =>
                                          x.pat_adm_id ===
                                          this.medicForm.value.pat_adm_id
                                  ).pat_adm_desc
                    ...
                    pat_weight: {
                        pat_weight_id: this.medicForm.value.pat_weight_id,
                        pat_weight_desc: this.listWeight.find(
                            x =>
                                x.pat_weight_id ===
                                this.medicForm.value.pat_weight_id
                        ).pat_weight_desc
                    },
                ...
                data => {
                    if (data.res_service === 'ok') {
                        ...
                        const bodyNotification = {
                            user_phone:
                                '+51' + this.basicForm.value.pat_apo_phone,
                            user_apo_names: this.basicForm.value.pat_apo_names,
                            user_names: this.basicForm.value.pat_names
                        };
                        this._patientService
                            .postNotification(bodyNotification)
                            .subscribe(
                                dataNotif => {
                                    if (dataNotif.res_service === 'ok') {
                                        this._matSnackBar.open(
                                            'Se notifico del registro correctamente al número ' +
                                                bodyNotification.user_phone,
                                            'Aceptar',
                                            {
                                                verticalPosition: 'top',
                                                duration: 2000
                ...
                dataDocument => {
                    if (dataDocument.res_service === 'ok') {
                        if (dataDocument.data_result.Count > 0) {
                            ...
                            this._patientService
                                .postSearchPatientPadron(bodySearch)
                                .subscribe(data => {
                                    if (data.res_service == 'ok') {
                                        if (data.data_result.Count > 0) {
                                            const patient_padron =
                                                data.data_result.Items[0];
                                            this.basicForm.controls[
                                                'pat_names'
                                            ].setValue(
                                                patient_padron.pat_names
                                            );
                                            this.basicForm.controls[
                                                'pat_ape_pat'
                                            ].setValue(
                                                patient_padron.pat_ape_pat
                                            );
                                            this.basicForm.controls[
                                                'pat_ape_mat'
                                            ].setValue(
                                                patient_padron.pat_ape_mat
                                            );
                                            this.basicForm.controls[
                                                'pat_apo_names'
                                            ].setValue(
                                                patient_padron.pat_apo_names
                                            );
                                            this.basicForm.controls[
                                                'pat_apo_ape_pat'
                                            ].setValue(
                                                patient_padron.pat_apo_ape_pat
                                            );
                                            this.basicForm.controls[
                                                'pat_apo_ape_mat'
                                            ].setValue(
                                                patient_padron.pat_apo_ape_mat
                                            );
                                            this.basicForm.controls[
                                                'pat_apo_num_doc'
                                            ].setValue(
                                                patient_padron.pat_apo_dni
                                            );
                                            this.basicForm.controls[
                                                'pat_apo_tip_doc'
                                            ].setValue('1');
                                            ...
                                            this.medicForm.controls[
                                                ...
                                                patient_padron.pat_his_cli
                                            );
                                            this.medicForm.controls[
                                                'pat_est_salud'
                                            ].setValue(
                                                patient_padron.pat_est_salud
                                            );
                                            this.locationForm.controls[
                                                'pat_dire'
                                            ].setValue(patient_padron.pat_dire);
                                        }
                                    }
                                });
                        }
